import { Container, Divider, Group, MediaQuery, RingProgress, Stack, Text } from '@mantine/core'
    ...
    <Container className={style.groupContainer} p={35} fluid>
        <Group position='apart'>
            <Stack className={style.groupStack} mt={10} mb={10}>
                <Text className={style.groupAmount} mt={-15}>{amount}</Text>
                <Text className={style.groupDescription} mt={-10}>{description}</Text>
            </Stack>
            <MediaQuery smallerThan="sm" styles={{ display: "none" }}>
                <Divider orientation='vertical' size={2} color={'white'} mr={10}/>
            </MediaQuery>
            <Stack className={style.groupStack} mt={10} mb={10} >
                <Text className={style.groupAmount} mt={-15}>{amount2}</Text>
                <Text className={style.groupDescription} mt={-10}>{description2}</Text>
            </Stack>
            <MediaQuery smallerThan="sm" styles={{ display: "none" }}>
                <Divider orientation='vertical' size={2} color={'white'} mr={10}/>
            </MediaQuery>
            <Stack className={style.groupStack} mt={10} mb={10}>
                <Text className={style.groupAmount} mt={-15}>{amount3}</Text>
                <Text className={style.groupDescription} mt={-10}>{description3}</Text>
            </Stack>
        </Group>
    </Container>;

    const renderProgressStatCard = type === 'progress' && 
    <Container className={style.container} p={25} fluid>
        <Group>
            <RingProgress
                size={110}
